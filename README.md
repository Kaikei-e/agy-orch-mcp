...
- **Clean Workspace Requirement**: `workspace.dirty_policy` is `"reject"` and requires a clean working repository (commit or stash uncommitted changes before invoking batch).
...
- **Bounded Retries, Deadlines & Clamping**: Batches enforce execution limits (up to 50 worker calls, 16 parallel tasks, 7,200,000ms / 2h wall time). Requested budget values exceeding server limits are automatically clamped and reported in `budget_adjustments` rather than rejected. Bounded execution loops are tracked by `budget.max_worker_calls`, `max_repair_attempts`, `max_replans`, and `wall_time_ms`. Periodic progress notifications (heartbeat) are emitted when a progressToken is supplied.
